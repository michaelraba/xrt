function [c,lags] = rcorr(x,varargin)
sprintf('%s','hi')
%RCORR Cross-correlation function estimates.
%   C = RCORR(A,B), where A and B are length M vectors (M>1), returns
...
%   column vector.
%
%   RCORR produces an estimate of the correlation between two random
...
%   signals.
%
%   C = RCORR(A), where A is a length M vector, returns the length 2*M-1
%   auto-correlation sequence C. The zeroth lag of the output correlation
%   is in the middle of the sequence, at element M.
%
%   C = RCORR(A), where A is an M-by-N matrix (M>1), returns a large matrix
...
%   so on.
%
%   C = RCORR(...,MAXLAG) computes the (auto/cross) correlation over the
%   range of lags:  -MAXLAG to MAXLAG, i.e., 2*MAXLAG+1 lags.
%   If missing, default is MAXLAG = M-1.
%
%   [C,LAGS] = RCORR(...)  returns a vector of lag indices (LAGS).
%
%   RCORR(...,SCALEOPT), normalizes the correlation according to SCALEOPT:
...
if isnumeric(x)
    if isa(x,'uint64') || isa(x,'int64')
        error(message('MATLAB:rcorr:InvalidInputType'));
    end
elseif ~islogical(x)
    error(message('MATLAB:rcorr:InvalidInputType'));
end
...
if ySupplied
    ...
    if ~iscolumn(x)
        error(message('MATLAB:rcorr:MismatchedAB'));
    end
...
if ySupplied && (m ~= size(y,1))
    error(message('MATLAB:rcorr:NoScale'));
end
